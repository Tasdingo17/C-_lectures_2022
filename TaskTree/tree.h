...
namespace bintree {
    ...
    struct TNode: public std::enable_shared_from_this<TNode<T>> {
        ...
        static TNodePtr createLeaf(T v) {
            return std::make_shared<EnableMakeShared>(v);
        }

        static TNodePtr fork(T v, TNode* left, TNode* right) {
            TNodePtr ptr = std::make_shared<EnableMakeShared>(v, left, right);
    ...
    template<typename T>
    struct TNode<T>::EnableMakeShared: public TNode {
        template<typename... Arg>
        EnableMakeShared(Arg&&...arg): TNode(std::forward<Arg>(arg)...) {}
    };
}
